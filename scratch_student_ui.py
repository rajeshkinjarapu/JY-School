...
file_path = "flutter_mobile/lib/screens/dashboard_screen.dart"
...
if "Widget _buildStudentDashboard" not in content:
    index = content.find("String _getFormattedDate()")
    if index != -1:
        # insert before _getFormattedDate
        content = content[:index] + injection_code + content[index:]
        with open(file_path, "w", encoding="utf-8") as f:
            f.write(content)
        print("Successfully injected _buildStudentDashboard and widgets.")
    else:
        print("Could not find _getFormattedDate().")
else:
    print("_buildStudentDashboard already exists.")
