...
- **Question Bank LaTeX Rendering Fixes:**
  ...
  - Adjusted the layout thresholds (`maxLen < 10` for 4-columns, `maxLen < 85` for 2-columns) so that visually narrow math fractions stay in a single line while wide formulas correctly wrap to a 2x2 grid.
- **Frontend Bug Fixes:**
  - Fixed a missing `api` import in `LoginPage.tsx` that could cause the login module to fail compilation and get stuck on the loading spinner during dynamic imports.

## Infrastructure & Hosting Migration (In Progress)
- **VPS Purchase:** The user successfully purchased a self-managed BigRock VPS (Ubuntu 22.04, NVMe 4, 2 Cores, 4GB RAM) for 1 year to resolve cold-start delays from free PaaS providers (Railway/Vercel).
- **Server IP:** `66.116.252.191`
- **Current Status:** 
  - The server is provisioned and running.
  - We have a planned implementation for setting up Node.js, PM2, Git, and Nginx.
  - The user experienced a `Permission denied` error when trying to SSH into the server due to a suspected typo in the password (`M2LuCO0fm)`).
  - The user is currently restarting their computer/terminal and requested to pause the session. **Waiting for the user to say "start" to resume the SSH login and server setup process.**
