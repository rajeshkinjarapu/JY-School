...
export const getDailySummary = async (req: AuthRequest, res: Response, next: NextFunction): Promise<void> => {
  ...
  const allMatchedClasses = await prisma.class.findMany({ select: { id: true, name: true, section: true, _count: { select: { students: true } } }});
  const allClasses = sortClasses(allMatchedClasses);
