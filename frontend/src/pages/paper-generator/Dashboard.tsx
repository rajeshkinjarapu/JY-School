...
export const Dashboard: React.FC = () => {
    ...
    return (
      <div className="min-h-screen bg-darkBg text-white flex items-center justify-center font-sans">
        <div className="flex flex-col items-center gap-3">
          <div className="w-12 h-12 border-4 border-t-accentIndigo border-slate-700 rounded-full animate-spin" />
  ...
  return (
    <div className="min-h-screen bg-slate-50 bg-gradient-to-tr from-slate-100 via-indigo-50/20 to-purple-50/20 text-slate-800 font-sans pb-12 relative overflow-hidden">
      ...
      <div className="absolute bottom-[-150px] left-[-150px] w-[600px] h-[600px] bg-accentTeal/4 rounded-full blur-[160px] pointer-events-none" />
      {/* Top Navigation */}
      <nav className="border-b border-slate-200 bg-white/75 backdrop-blur-md sticky top-0 z-40 text-slate-850">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 bg-gradient-to-tr from-accentIndigo to-accentPurple rounded-xl flex items-center justify-center">
          ...
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-2 px-3 py-1.5 bg-slate-100 rounded-xl border border-slate-200">
              <User className="w-4 h-4 text-accentIndigo" />
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-8 relative z-10">
        ...
        <div className="bg-white rounded-3xl p-6 border border-slate-200 shadow-md mb-8 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          ...
          <div className="flex gap-3">
            ...
            >
              <Database className="w-4 h-4 text-accentTeal" />
              <span>Question Bank</span>
            </Link>
            <Link
              to="/papers/new"
              className="px-5 py-2.5 bg-gradient-to-r from-accentIndigo to-accentPurple hover:brightness-110 text-white rounded-xl text-sm font-medium shadow-md hover:shadow-lg transition-all flex items-center gap-2"
            >
        ...
        <div className="bg-white border border-slate-250 rounded-3xl p-6 shadow-sm">
          ...
          {papers.length === 0 ? (
            <div className="text-center py-16 bg-slate-50 rounded-2xl border border-dashed border-slate-300">
              ...
              <Link
                to="/papers/new"
                className="mt-5 inline-flex items-center gap-2 px-5 py-2 bg-accentIndigo hover:bg-indigo-600 text-white rounded-xl text-xs font-semibold shadow-sm transition-all"
              >
                <PlusCircle className="w-4 h-4" />
              ...
              {papers.map((paper) => {
                ...
                return (
                  <Link
                    key={paper.id}
                    to={`/papers/${paper.id}`}
                    className="group bg-slate-50/50 hover:bg-slate-50 border border-slate-200 hover:border-accentIndigo/30 rounded-2xl p-5 shadow-sm transition-all hover:shadow-md flex flex-col justify-between relative overflow-hidden"
                  >
                    {/* Glowing highlight decoration on hover */}
                    <div className="absolute top-0 right-0 w-24 h-24 bg-accentIndigo/5 rounded-full blur-xl group-hover:bg-accentIndigo/10 transition-colors" />

                    <div>
                      <div className="flex justify-between items-center mb-3">
                        <span className="bg-slate-200 group-hover:bg-indigo-50 border border-slate-250 text-slate-650 group-hover:text-accentIndigo text-[10px] font-bold font-mono px-2 py-0.5 rounded uppercase transition-colors">
                      ...
                      <h3 className="text-base font-bold text-slate-800 group-hover:text-accentIndigo line-clamp-1 pr-4 transition-colors">
                        {paper.title}
                      </h3>
                    ...
                    <div className="flex justify-between items-center mt-5 border-t border-slate-200/60 pt-3">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-accentIndigo group-hover:translate-x-1.5 transition-transform">
