...
export const TimetablePage: React.FC = () => {
  ...
  // Configs
  const [configs, setConfigs] = useState<Record<string, PeriodConfig[]>>({
    PRIMARY: [],
    HIGHER: [],
  });
  ...
  const fetchConfigs = async () => {
    try {
      ...
      const grouped: Record<string, PeriodConfig[]> = {
        PRIMARY: [],
        HIGHER: [],
      };
      all.forEach((c: any) => {
      ...
      Object.keys(grouped).forEach((k) =>
        grouped[k].sort((a, b) => a.periodNumber - b.periodNumber),
      );
      setConfigs(grouped);
    } catch {
      /* ignore */
  ...
  const loadAllSlots = async () => {
    ...
    try {
      const responses = await Promise.all(
        classes.map((c) => api.get(`/api/timetable/class?classId=${c.id}`)),
      );
      const all: SlotData[] = [];
  ...
  useEffect(() => {
    if (activeTab === "workload" && classes.length) loadAllSlots();
  }, [activeTab, classes]);
  useEffect(() => {
    if (activeTab === "settings") {
      setEditPeriods(
        configs[settingsCategory]?.length
  ...
  useEffect(() => {
    if (modalClassId) {
      api
        .get(`/api/classes/${modalClassId}/subjects`)
  ...
  const handleAutoGenerate = async () => {
    setGenerating(true);
    try {
      const res: any = await api.post("/api/timetable/generate-auto", {
        clearExisting,
      });
          ...
          {activeTab === "teacher" && (
            <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-150 dark:border-gray-800 overflow-hidden p-4 print:p-0">
              ...
              <div className="hidden print:block text-center mb-6">
                ...
                <h2 className="text-lg font-bold text-gray-800 mt-1">
                  Teacher Timetable —{" "}
                  {teachers.find((t) => t.id === selectedTeacherId)?.user
                    .name || ""}
                </h2>
                <p className="text-xs text-gray-400 mt-0.5">
                ...
                <table className="w-full text-left border-collapse text-sm">
                  <thead>
                    <tr className="border-b border-gray-200 dark:border-gray-800 bg-gray-50 dark:bg-gray-800/50">
                      <th className="p-3 font-extrabold text-gray-400 text-[10px] uppercase tracking-widest">
                        Teacher
                      </th>
                      <th className="p-3 font-extrabold text-gray-400 text-[10px] uppercase tracking-widest">
                        Employee ID
                      ...
                      <th className="p-3 font-extrabold text-gray-400 text-[10px] uppercase tracking-widest w-60">
                        Load Gauge
                      </th>
                      <th className="p-3 font-extrabold text-gray-400 text-[10px] uppercase tracking-widest text-right">
                        Status
                      </th>
                    </tr>
                  </thead>
                    ...
                    {teachers.map((t) => {
                      ...
                      const uniqueClasses =
                        [
                          ...new Set(
                            tSlots.map(
                              (s) => `${s.class?.name}-${s.class?.section}`,
                            ),
                          ),
                        ].join(", ") || "—";
                      return (
                        <tr
                          key={t.id}
                          className="hover:bg-gray-50/50 dark:hover:bg-gray-800/10"
                        >
                          <td className="p-3">
                          ...
                          <td className="p-3 text-sm font-extrabold text-gray-800 dark:text-gray-200">
                            {count}
                          </td>
                          <td
                            className="p-3 text-[10px] text-gray-500 font-semibold max-w-[200px] truncate"
                            title={uniqueClasses}
                          >
                            {uniqueClasses}
                          </td>
                          <td className="p-3">
                            <div className="w-full bg-gray-100 dark:bg-gray-800 rounded-full h-2.5 overflow-hidden">
                              <div
                                className={`h-full rounded-full transition-all duration-500 ${count > 30 ? "bg-red-500" : count >= 20 ? "bg-emerald-500" : "bg-amber-500"}`}
                                style={{ width: `${pct}%` }}
                              />
                            </div>
            ...
            <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-150 dark:border-gray-800 p-6 space-y-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-gray-400 mt-1">
                    Define how many periods per day, timings, and break slots
                    for each category.
                  </p>
                </div>
              ...
              <div className="space-y-2">
                ...
                {editPeriods.map((p, idx) => (
                  <div
                    key={idx}
                    className={`grid grid-cols-12 gap-3 items-center p-2 rounded-lg ${p.isBreak ? "bg-amber-50/50 dark:bg-amber-950/10 border border-amber-200/50 dark:border-amber-900/20" : "bg-gray-50 dark:bg-gray-800/30 border border-gray-100 dark:border-gray-800"}`}
                  >
                    <div className="col-span-1 text-sm font-extrabold text-gray-400">
                      {idx + 1}
                    </div>
                    <div className="col-span-3">
                      <input
                        value={p.label}
                        onChange={(e) =>
                          updatePeriodRow(idx, "label", e.target.value)
                        }
                    ...
                    <div className="col-span-2">
                      <input
                        ...
                        value={p.startTime}
                        onChange={(e) =>
                          updatePeriodRow(idx, "startTime", e.target.value)
                        }
                        className="w-full bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-lg px-2.5 py-1.5 text-sm font-bold outline-none focus:ring-2 focus:ring-primary-500/20"
                      />
                    </div>
                    <div className="col-span-2">
                      <input
                        ...
                        value={p.endTime}
                        onChange={(e) =>
                          updatePeriodRow(idx, "endTime", e.target.value)
                        }
                        className="w-full bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-lg px-2.5 py-1.5 text-sm font-bold outline-none focus:ring-2 focus:ring-primary-500/20"
                      />
                    </div>
                    ...
                    <div className="col-span-2 text-right">
                      <button
                        ...
                        className="p-1.5 rounded-lg hover:bg-red-50 dark:hover:bg-red-950/20 text-gray-400 hover:text-red-500 transition-colors cursor-pointer"
                      >
            ...
            <div className="flex items-center justify-between">
              <div>
                <h3 className="text-lg font-extrabold">
                  {editingSlot ? "Edit Slot" : "Assign Slot"}
                </h3>
                <p className="text-xs text-gray-400">
                  {modalDay} · Period {modalPeriod}
                </p>
              </div>
              <button
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <input
                    ...
                    value={modalPeriod}
                    onChange={(e) =>
                      setModalPeriod(parseInt(e.target.value) || 1)
                    }
                    className="input"
                    required
                  />
          ...
          <div className="card w-full max-w-md p-6 space-y-5">
            <div className="flex justify-between items-center">
              <h3 className="text-lg font-bold flex items-center gap-2">
                <Wand2 className="w-5 h-5 text-primary-500" /> Auto Generate
                Timetable
              </h3>
              <button
            ...
            <div className="space-y-4">
              <div className="p-4 rounded-xl bg-amber-50 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-900/40">
                <p className="text-xs font-semibold text-amber-800 dark:text-amber-300 leading-relaxed">
                  <strong>How it works:</strong> The auto-generator reads all
                  Class → Subject → Teacher mappings (from Subjects page) and
                  your Period Settings configuration, then assigns subjects to
                  periods across Monday–Saturday while avoiding teacher
                  conflicts.
                </p>
              </div>
                ...
                <label className="flex items-center gap-3 cursor-pointer select-none">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-gray-400 mt-0.5">
                      If unchecked, auto-generation will only fill empty slots
                    </p>
                  </div>
                </label>
              </div>
              ...
              {clearExisting && (
                <div className="p-3 rounded-lg bg-red-50 dark:bg-red-950/20 border border-red-200 dark:border-red-900/40 text-xs font-semibold text-red-700 dark:text-red-300">
                  ⚠️ Warning: This will delete ALL existing timetable entries
              ...
              <div className="text-xs text-gray-500 font-semibold space-y-1">
                <p>
                  ✓ Uses Period Settings (PRIMARY / HIGHER) for correct period
                  structure
                </p>
