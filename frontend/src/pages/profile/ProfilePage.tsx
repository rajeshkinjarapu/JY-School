...
export const ProfilePage: React.FC = () => {
  ...
  const [editQual, setEditQual] = useState(
    (user as any)?.teacher?.qualification || "",
  );
  const [editJoinDate, setEditJoinDate] = useState(
    (user as any)?.teacher?.joiningDate
      ? new Date((user as any)?.teacher?.joiningDate)
          .toISOString()
          .split("T")[0]
      : "",
  );
  ...
  const handlePasswordSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.error(
        error.message || "Error changing password. Verify old password.",
      );
    } finally {
      setIsSaving(false);
    }
            ...
            <form onSubmit={handleProfileSave} className="space-y-4 mt-4">
              ...
              <div className="flex justify-end gap-3 pt-2">
                ...
                <button
                  type="submit"
                  disabled={isSaving}
                  className="px-4 py-2 text-sm font-bold text-white bg-indigo-600 hover:bg-indigo-700 rounded-lg shadow disabled:opacity-50"
                >
                  ...
                  {user.role === "TEACHER" && (
                    <>
                      <div>
                        ...
                        <span className="font-semibold text-gray-900 dark:text-white">
                          {(user as any).teacher?.qualification || "N/A"}
                        </span>
                      </div>
                      <div>
                        <span className="text-xs text-gray-400 block">
