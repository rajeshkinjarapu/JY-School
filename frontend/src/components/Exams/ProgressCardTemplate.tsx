...
export const ProgressCardTemplate: React.FC<ProgressCardTemplateProps> = ({ 
  ...
  const infoRows = [
    ...
    { icon: '📖', label: 'SECTION', value: safeData.section },
    { icon: '📞', label: 'MOBILE', value: safeData.mobile },
  ...
  return (
    <div
      ...
      style={{
        ...
        overflow: 'hidden',
        border: '2px solid #0b1a33', // outer dark blue
    ...
    >
      ...
      <div style={{ display: 'flex', alignItems: 'center', padding: '24px 32px 12px 32px', gap: 16 }}>
        ...
        <div style={{ textAlign: 'center', flex: 1, padding: '0 8px' }}>
          ...
          {settings?.schoolSubtitle && (
            <div style={{ fontSize: 13, color: '#64748b', letterSpacing: 0.8, margin: '2px 0', whiteSpace: 'nowrap', fontWeight: 500 }}>
              {settings.schoolSubtitle}
            </div>
          )}
          <div style={{ fontSize: 12, color: '#64748b', marginTop: 2, whiteSpace: 'nowrap', fontWeight: 500 }}>
            {settings?.schoolAddress || "Opp. Hero Showroom, SVL Paradise Campus, Narasannapeta"}
          </div>
          <div style={{ fontSize: 20, fontWeight: 500, color: '#64748b', letterSpacing: 2, margin: '5px 0 0', textTransform: 'uppercase', whiteSpace: 'nowrap' }}>
            {examTitle}
          </div>
          <div style={{ fontSize: 16, fontWeight: 500, color: '#f39c12', letterSpacing: 4, marginTop: 2, whiteSpace: 'nowrap', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
            <span>✦</span> RESULT CARD <span>✦</span>
          </div>
        </div>
        <div style={{ width: 90, flexShrink: 0 }} />
      </div>

      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 14, padding: '0px 32px 12px 32px' }}>
        <div style={{ flex: 1, height: 2, background: 'linear-gradient(90deg,transparent,#f39c12,transparent)' }} />
      </div>

      <div style={{ margin: '0 24px 14px 24px', border: '1.5px solid #f39c12', borderRadius: 12, overflow: 'hidden', background: '#ffffff', display: 'flex', position: 'relative' }}>
        <div style={{ flex: 1 }}>
          {infoRows.map((row, i) => (
            <div key={i} style={{ display: 'grid', gridTemplateColumns: '175px 1fr', borderBottom: i < infoRows.length - 1 ? '1px solid #f1f5f9' : 'none', background: i % 2 === 1 ? '#f8fafc' : '#ffffff' }}>
              <div style={{ padding: '11px 16px', fontWeight: 700, fontSize: 12, color: '#0b1a33', display: 'flex', alignItems: 'center', gap: 8, whiteSpace: 'nowrap' }}>
        ...
        <div style={{ position: 'absolute', right: 14, top: 12 }}>
          {resolveUrl(safeData.photo) ? (
             <div style={{ padding: 2, background: '#fff', border: '2px solid #f39c12', borderRadius: 8, boxShadow: '0 4px 10px rgba(243,156,18,0.1)' }}>
                <img src={resolveUrl(safeData.photo)} alt="Student" style={{ width: 84, height: 102, objectFit: 'cover', borderRadius: 6 }} />
             </div>
          ) : (
      ...
      <div style={{ margin: '24px 24px 16px 24px' }}>
        <div style={{ fontSize: 15, fontWeight: 800, color: '#0b1a33', marginBottom: 8, display: 'flex', alignItems: 'center', gap: 10 }}>
          <span style={{ fontSize: 20 }}>📊</span>
          <span>Performance Summary</span>
          <span style={{ marginLeft: 'auto', fontSize: 12, fontWeight: 500, color: '#94a3b8' }}>Max Marks: {TOTAL_MAX_MARKS}</span>
        </div>
        <table style={{ width: '100%', borderCollapse: 'collapse', borderRadius: 8, overflow: 'hidden', boxShadow: '0 2px 8px rgba(0,0,0,0.02)', fontSize: 14, border: '1px solid #e2e8f0' }}>
          <thead>
            <tr style={{ background: '#0b1a33' }}>
              <th style={{ color: '#fff', fontWeight: 700, textTransform: 'uppercase', letterSpacing: 0.5, padding: '12px 16px 12px 20px', textAlign: 'left', fontSize: 12 }}>Subject</th>
              <th style={{ color: '#fff', fontWeight: 700, textTransform: 'uppercase', letterSpacing: 0.5, padding: '12px 16px', textAlign: 'center', fontSize: 12 }}>Marks</th>
              <th style={{ color: '#fff', fontWeight: 700, textTransform: 'uppercase', letterSpacing: 0.5, padding: '12px 16px', textAlign: 'center', fontSize: 12 }}>Max Marks</th>
              <th style={{ color: '#fff', fontWeight: 700, textTransform: 'uppercase', letterSpacing: 0.5, padding: '12px 16px', textAlign: 'center', fontSize: 12 }}>%</th>
            </tr>
          </thead>
          <tbody>
            {safeData.marks.map((sub: any, i: number) => {
              ...
              return (
                <tr key={i} style={{ background: i % 2 === 1 ? '#f8fafc' : '#ffffff', borderBottom: '1px solid #f1f5f9' }}>
                  <td style={{ padding: '14px 16px 14px 20px', textAlign: 'left', fontWeight: 700, color: '#0b1a33', textTransform: 'uppercase' }}>
                    <span style={{ display: 'inline-block', width: 8, height: 8, background: '#3b82f6', marginRight: 10, verticalAlign: 'middle', borderRadius: 2 }}></span>
                    {sub.subject}
                  </td>
            ...
            <tr style={{ background: '#fffcf5' }}>
              <td style={{ padding: '14px 16px 14px 20px', textAlign: 'left', fontWeight: 900, color: '#0b1a33', textTransform: 'uppercase', letterSpacing: 1, fontSize: 14 }}>
                <span style={{ color: '#dc2626', marginRight: 8, fontSize: 16 }}>📌</span> TOTAL
              </td>
              <td style={{ padding: '14px 16px', textAlign: 'center', fontWeight: 900, fontSize: 18, color: '#dc2626' }}>{safeData.total}</td>
              <td style={{ padding: '14px 16px', textAlign: 'center', color: '#1e3a8a', fontWeight: 700 }}>{TOTAL_MAX_MARKS}</td>
              <td style={{ padding: '14px 16px', textAlign: 'center', fontWeight: 900, color: '#1e3a8a' }}>{totalPct}%</td>
            </tr>
          </tbody>
        </table>
      </div>

      <div style={{ margin: '0 24px 14px 24px', background: '#ffffff', border: '1px solid #e2e8f0', borderRadius: 12, padding: '24px 24px 16px 24px', boxShadow: '0 2px 8px rgba(0,0,0,0.02)' }}>
        <div style={{ height: 16, background: '#f1f5f9', borderRadius: 20, overflow: 'hidden', border: '1px solid #e2e8f0' }}>
          <div style={{ height: '100%', width: `${barWidth}%`, background: 'linear-gradient(90deg,#1e3a8a,#60a5fa)', borderRadius: 20 }} />
        </div>
        <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 10, fontSize: 12, fontWeight: 500, color: '#64748b' }}>
          <span>0</span>
      ...
      <div style={{ margin: 'auto 24px 0 24px', paddingTop: 16, paddingBottom: 16, borderTop: '2px dashed #e2e8f0', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end' }}>
        <div>
          <div style={{ fontSize: 14, fontWeight: 800, color: '#1e3a8a', textTransform: 'uppercase', letterSpacing: 0.5, display: 'flex', alignItems: 'center', gap: 6 }}>
            <span style={{ fontSize: 16 }}>📋</span> TOTAL MARKS: {safeData.total} / {TOTAL_MAX_MARKS}
          </div>
          <div style={{ fontSize: 48, fontWeight: 900, color: '#dc2626', lineHeight: 1, letterSpacing: -1, marginTop: 4 }}>
            {totalPct}%
          </div>
        </div>
        <div style={{ display: 'flex', gap: 36 }}>
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', width: 130 }}>
            ...
            <div style={{ fontSize: 12, fontWeight: 700, color: '#0b1a33', textAlign: 'center', display: 'flex', alignItems: 'center', gap: 4 }}>✍ Teacher Signature</div>
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', width: 130 }}>
            ...
            <div style={{ fontSize: 12, fontWeight: 700, color: '#0b1a33', textAlign: 'center', display: 'flex', alignItems: 'center', gap: 4 }}>✍ Principal Signature</div>
          </div>
        </div>
      </div>

      <div style={{ background: '#0b1a33', color: '#94a3b8', fontSize: 10, textAlign: 'center', padding: '8px 8px', fontWeight: 500, letterSpacing: 0.5 }}>
