...
export const PrintablePaper: React.FC<PrintablePaperProps> = ({
    ...
    const renderLogo = () => {
        ...
        // Default SVG logo – matches the image exactly
        return (
            <svg viewBox="0 0 100 100" className="w-full h-full text-black">
                ...
                <polygon points="50,22 78,36 50,50 22,36" fill="none" stroke="currentColor" strokeWidth="1.2" />
                <line x1="50" y1="50" x2="50" y2="70" stroke="currentColor" strokeWidth="1.5" />
                <polygon points="42,70 58,70 50,78" fill="currentColor" />
                <text
                    x="50"
                    y="62"
                    fontSize="9"
    ...
    return (
        ...
        >
            <style dangerouslySetInnerHTML={{ __html: `
                ...
                .a4-paper .question-option {
                    display: inline-flex;
                    align-items: baseline;
                    gap: 2px;
                }
                .a4-paper .option-letter {
                    font-weight: 700;
                    margin-right: 2px;
                    flex-shrink: 0;
                }
            ...
            {!showSolutions && !showAnswerKey && (
                <div className="z-10 relative">
                    ...
                    <div className="flex items-center gap-4 border-b-2 border-black pb-3 mb-3 font-sans text-black">
                        ...
                        <div className="flex-1 text-center">
                            ...
                            <div className="text-xs md:text-sm font-serif italic text-slate-800 leading-tight mt-1">
                                {paper.subHeader1 !== undefined && paper.subHeader1 !== null
                                    ? paper.subHeader1
                                    : '(IIT-JEE/NEET Foundation – Olympiads)'}
                            </div>
                            <div className="text-xs md:text-sm font-serif text-slate-700 leading-tight">
                                {paper.subHeader2 !== undefined && paper.subHeader2 !== null
                                    ? paper.subHeader2
                                    : 'Opp. Hero Showroom, SVL Paradise Campus, Narasimhapatna'}
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-y-1 mb-4 font-sans text-xs md:text-sm font-bold border-b border-black pb-2 text-black leading-tight">
                        <div>Class – {paper.className || '7th'}</div>
                        <div className="text-right">Marks : {paper.totalMarks}</div>
                        <div>Date : {paper.examDate || '29-08-2026'}</div>
                        <div className="text-right">
                            Duration : {paper.duration} Minutes
                            {paper.paperCode && (
                                <span className="font-mono text-indigo-600 ml-1">({paper.paperCode})</span>
                            )}
                        </div>
                    </div>
                    ...
                    <div className="border border-black p-3 mb-6 font-sans text-xs grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <span className="inline-block w-[60%] border-b border-black/30" />
                        </div>
                        <div>
                            ...
                            <div key={section.id} className="mb-8">
                                ...
                                <div className={isDoubleColumn ? 'two-column-layout relative' : 'space-y-3 relative'}>
                                    {section.questions.map((q) => {
                                        ...
                                        return (
                                            <div key={q.id} className="question-block no-break">
                                                {/* Question text */}
                                                <div className="flex items-start gap-1.5 mb-1.5 leading-snug whitespace-pre-wrap">
                                                    <span className="font-sans font-bold text-xs bg-slate-100 border border-slate-300 rounded px-1.5 py-0.5 mr-2 flex-shrink-0">
                                                        {qNum}.
                                                    </span>
                                                    <div className="flex-1">
                                                        <LaTeXPreview
                                                            text={q.questionText}
                                                            className="paper-serif font-serif text-[14px]"
                                                        />
                                                    </div>
                                                </div>
                                                        ...
                                                        {[q.optionA, q.optionB, q.optionC, q.optionD].map(
                                                            (opt, idx) => (
                                                                <div key={idx} className="question-option">
                                                                    <span className="option-letter mr-1.5 font-bold">
                                                                        {getOptionLabel(idx)}
                                                                    </span>
            ...
            {showOmr && !showSolutions && !showAnswerKey && (
                <div className="page-break z-10 relative pt-6 font-sans">
                    <div className="border-2 border-black p-6 rounded-xl">
                        <h2 className="text-center font-bold text-lg mb-2 tracking-wider">
                            OMR RESPONSE GRID
                        </h2>
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-4 gap-6">
                            {Array.from({ length: Math.ceil((absoluteQuestionCounter - 1) / 10) }).map(
                                (_, colIdx) => (
                                    <div key={colIdx} className="space-y-3">
                                        ...
                                        {Array.from({ length: 10 }).map((_, rowIdx) => {
                                            ...
                                            return (
                                                <div
                                                    key={qNo}
                                                    className="grid grid-cols-5 items-center text-center text-xs"
                                                >
                                                    <div className="font-bold text-slate-600 font-mono">
                                                        {qNo}
                                                    </div>
                                                    <div>
                                                        <span className="omr-bubble inline-block w-4 h-4 border border-black rounded-full" />
                                                    </div>
                                                    <div>
                                                        <span className="omr-bubble inline-block w-4 h-4 border border-black rounded-full" />
                                                    </div>
                        ...
                        <table className="w-full border-collapse border border-black text-center text-sm">
                            <thead>
                                <tr className="bg-slate-100">
                                    <th className="border border-black p-2 font-bold">Q.No</th>
                                    <th className="border border-black p-2 font-bold">Subject</th>
                                    <th className="border border-black p-2 font-bold">Key</th>
                                </tr>
                            </thead>
                            <tbody>
                                {(() => {
