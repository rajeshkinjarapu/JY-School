...
export const PrintablePaper: React.FC<PrintablePaperProps> = ({
  ...
  return (
    <div id="a4-preview-paper" className="bg-white text-black p-8 border border-slate-300 mx-auto max-w-[21cm] relative shadow-2xl font-serif lining-nums tabular-nums text-[14px] leading-relaxed select-text print:p-0 print:border-none print:shadow-none">
      {/* Watermark */}
                      ...
                      {q.type.startsWith('MCQ') && (
                        <div className={`${
                          Math.max((q.optionA || '').length, (q.optionB || '').length, (q.optionC || '').length, (q.optionD || '').length) < 20
                            ...
                            : Math.max((q.optionA || '').length, (q.optionB || '').length, (q.optionC || '').length, (q.optionD || '').length) < 45
