...
export const GatePassPrint: React.FC<GatePassPrintProps> = ({ gatePass, schoolName = 'JY SCHOOL' }) => {
  ...
  return (
    <>
      ...
      <div className="hidden print:flex flex-col w-[210mm] h-[296mm] mx-auto text-slate-900 bg-white overflow-hidden box-border">
        {/* 2 Copies (Security Copy & Person Copy) */}
        {[ 'SECURITY COPY', 'VISITOR COPY' ].map((copyType, idx) => (
          <div key={copyType} className={`flex-1 flex flex-col justify-center p-8 ${idx === 1 ? 'border-t-[3px] border-dashed border-slate-300' : ''}`}>
            <div className="w-full border-2 border-slate-900 p-6 rounded-lg relative overflow-hidden bg-white">
              
              {/* Header */}
              <div className="flex justify-between items-center border-b-2 border-slate-900 pb-4 mb-4">
                <div className="flex items-center gap-4">
                  <div className="w-16 h-16 rounded-full border-2 border-slate-900 flex items-center justify-center overflow-hidden bg-white shrink-0">
                ...
                <div className="text-right">
                  ...
                  <div className="text-xs font-semibold text-slate-600">
                    Date: {gatePass.createdAt ? format(new Date(gatePass.createdAt), 'dd MMM yyyy') : '-'}
                  </div>
                </div>
              </div>
              ...
              <div className="flex gap-6">
                ...
                <div className="flex-1 grid grid-cols-2 gap-y-4 gap-x-8 text-sm">
                  <div>
                    ...
                    <div className="font-bold border-b border-slate-300 pb-1 truncate">{personName || 'N/A'}</div>
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <div className="font-bold border-b border-slate-300 pb-1 truncate">{gatePass.destination || 'N/A'}</div>
                  </div>

                  <div className="col-span-2">
                    <div className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-0.5">Reason</div>
                    <div className="font-bold border-b border-slate-300 pb-1 truncate">{gatePass.reason || 'N/A'}</div>
                  </div>
