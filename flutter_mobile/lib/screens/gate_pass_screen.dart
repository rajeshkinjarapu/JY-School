...
class _DashboardTab extends StatefulWidget {
  ...
  const _DashboardTab({required this.isStudent});
  @override
...
class _DashboardTabState extends State<_DashboardTab> with TickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    ...
    final isStudentTab = widget.isStudent ? true : _tabController.index == 0;
...
class _LiveListTab extends StatefulWidget {
  final bool isStudent;
  const _LiveListTab({required this.isStudent});
  @override
...
class _LiveListTabState extends State<_LiveListTab> {
  ...
  Widget build(BuildContext context) {
    ...
    final studentPasses = _activePasses.where((p) => p['requestType'] == 'STUDENT').toList();
    final count = widget.isStudent ? studentPasses.length : _activePasses.length;

    Widget listBody;
    if (widget.isStudent) {
      listBody = _buildList(studentPasses, true);
    } else {
      listBody = TabBarView(
        children: [
        ...
        Container(
          ...
          child: Row(
            children: [
              ...
              Text(
                '$count people currently OUTSIDE campus',
                style: GoogleFonts.poppins(
...
class _ApprovalsTabState extends State<_ApprovalsTab> {
  ...
  Widget build(BuildContext context) {
    ...
    final studentPasses = _pendingPasses.where((p) => p['requestType'] == 'STUDENT').toList();
    final count = widget.isStudent ? studentPasses.length : _pendingPasses.length;
...
class _HistorySearchTabState extends State<_HistorySearchTab> {
  ...
  Widget build(BuildContext context) {
    ...
    Widget listBody;
    if (widget.isStudent) {
      listBody = _buildList(_filtered, true, statusColor);
    } else {
      listBody = TabBarView(
        children: [
